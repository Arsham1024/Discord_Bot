...
def database_control(command, key, value=None):
    ...
    if command in ['del', 'add', 'get']:
        ...
        output_msg = 'Invalid command, please try again.'
    return output_msg
...
async def on_message(message):
    async def help():
        ...
        # !help [command]
            ...
            commands = {"8ball": [" `[question]` Ask the magic 8 ball a question and get a response.",
                                  "Ask the magic 8 ball a question (or anything else) and 							get a response from a list of possible responses",
                                  "`[question]` - 								Anything the user inputs (although makes more sense to be a question)							",
                                  "N/A"],
                        "fact": [" `[number]` `[type='trivia']` - Returns a fact from the NumbersAPI.",
                                 "Access a string from URL: 'http://numbersapi.com/' + number + '/' + type + '?json' and sends it as a message to the channel.",
                                 "`[Number]` - must be (1) any float/int value or (2) the word 'random'.\n`[Type]` - must be the word (1) 'year', (2) 'date', (3) 'math', or (3) 'trivia'.",
                                 "N/A"],
                        "play": [" - Starts up a playable game in Discord.",
                                 "Creates (or accesses) a game state from the database and displays it as an Embed message.",
                                 "N/A",
                                 "User reactions will be recorded and used to change game state saved to the database. Previous game state message will be deleted and a new message sent."],
                        "restricted": [" - A moderation tool to create restricted words.",
                        ...
                        "restricted list": [" - Displays the list of restricted words.",
                                            ...
                                            "N/A", "!restricted"],
                        "restricted add": [" `[word]` - Adds a word to the list of restricted words.",
                                           ...
                                           "`[word]` - any word (or number technically)", "!restricted"],
                        "restricted delete": [" `[index]` -  Deletes a word from the list of restricted words.",
                                              ...
                                              "`[index]` - Valid index", "!restricted"],
                        "poll": [" `-[prompt]` `-[choice1]` `-[choice2]` `-[choice3]` ... - Creates a poll.",
                                 "Creates a poll with prompt being the question and the choices being the available choices. When the poll is created, users can vote by reacting to the poll message. The bot then uses the gathered data to create a bar graph with correct labels to display to the user.",
                                 "`[prompt]` - The prompt is specified immediately after the command “!poll-” is called. This is the question to be displayed to the users.\n`[choice list] (1-20)` - delimited by “-” specify all the choices for any given poll question. Currently supports as many as 20 possible choices.",
                                 "N/A"],
                        "disable": [" - Shuts Gonzo down.",
                                    "Displays a simple goodbye message to the channel. Gonzo goes offline and is no longer active.",
                                    "N/A", "N/A"],
                        "db": [" - Displays database.", "Displays the values currently in the database for the server.",
                               "N/A", "N/A"],
                        "translate": ["`[message]` - Displays the Spanish translation of an English message.",
                                      "Displays the Spanish translation of a message of an English message.",
                                      "`[message]` - an english phrase", "N/A"],
                        "lyrics": ["`[Song Name] by [Artist]` - Displays the lyrics of a song.",
                                   "Displays the lyrics of a song.",
                                   "`[Song Name]` - Name of a valid song\n`[Artist]` - Name of the artist who sang the song",
                                   "N/A"],
                        "weather": ["`[City Name]` - Displays the current weather in a city.",
                                    ...
                                    "`[City Name]` - A valid city", "N/A"],
                        "hello": [" - Displays a hello message.", "Displays a simple hello message in the channel.",
                                  "N/A", "N/A"],
                        "thanks": [" - Displays a message of gratitude.",
                                   "Displays a simple response from our bot to the channel. Use when you want to say thank you to Gonzo. :)",
                                   "N/A", "N/A"],
                        "remind": ["`[amount of time and unit] [reminder]` - Reminds the user to do something.",
    ...
    async def remind():
        ...
        if len(message_array) <= 2:
            await message.channel.send(
                "**Warning**: Arguments were not entered corrently. Please use `!help remind` for more information.")
            return
        ...
        if converted_time < 0:
            await message.channel.send(
                "**Warning**: Time was not entered corrently. Please use `!help remind` for more information.")
            return
    ...
    async def play():
        ...
        def initialize_game(author):
            ...
            new_board = [10 * border, border + 8 * floor + border, border + 8 * floor + border,
                         border + 8 * floor + border, border + 8 * floor + border, border + 8 * floor + border,
                         border + 8 * floor + border, border + 8 * floor + border, border + 8 * floor + border,
                         10 * border]
        ...
        def player_move(author, reaction):
            ...
            try:
                if reaction == '\U00002B06':
                    row -= 1  # up
                elif reaction == '\U00002B07':
                    row += 1  # right
                elif reaction == '\U000027A1':
                    col += 1  # down
                elif reaction == '\U00002B05':
                    col -= 1  # left
    ...
    async def weather():
        ...
        if (location_response.status_code == 200):
            ...
            weather_embed = Embed(title=city_search_result,
                                  description=str(round(weather_temp)) + "° F " + "and " + weather_state_name,
                                  color=Color.orange())
            print(icon)
    ...
    async def translate():
        ...
        target_data = requests.post('https://translate.argosopentech.com/translate',
                                    data={'q': source_text, 'source': 'en', 'target': 'es'})
    ...
    async def eight_ball():
        if len(parsed_message) == 1:
            ...
            return
        eight_ball_responses = ["Yes", "No", "Definitely", "Signs point to yes", "Better not tell you now",
                                "It is doubtful", "Do not rely on it"]
    ...
    async def restricted():
        ...
        if parsed_message[1] == "list":
            try:
                ...
                restricted_list = Embed(title="List of Restricted Words. May contain offensive language",
                                        description="".join(list_of_words), color=0x008080)
    ...
    async def restrictedWordFound():
        ...
        warning = Embed(title="@{}".format(message.author), description="||{}||".format(message.content),
                        color=0x008080)
